    ...
    <select
      ...
      <InputError :message="$page.props?.errors.brand_car" />
		...
		<div class="relative z-0 mb-6 w-full group">
    		...
    		<div class="relative z-0 mb-6 w-full group">
			<textarea v-model="form.notes" type="text" rows="4" name="floating_notes" autocomplete="off" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-cyan-500 focus:outline-none focus:ring-0 focus:border-cyan-600 peer" placeholder=" " />
...
export default {
  ...
  methods: {
     store() {
      this.form.post(route('guest.store'));
    },
  },
};
</script>
